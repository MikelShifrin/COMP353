...
<html lang = "US-en">
    ...
    <body>
        ...
        <div class="jumbotron">
          <h1 class="display-3">Report 5</h1>
          ...
          <div class=container>
              <code>
                  ...
                  WHERE uID IN ((<br/>
                  (SELECT Ad.bID FROM Ad, Books<br/>
                  WHERE Ad.bID IS NOT NULL AND Books.bID = Ad.bID IN (<br/>
                  SELECT bID, rating FROM Books ORDER BY rating LIMIT 1 ) OR<br/>
                  (SELECT Ad.cID FROM Ad, Clothes<br/>
                  WHERE Ad.cID IS NOT NULL AND Clothes.cID = Ad.cID IN (<br/>
                  SELECT cID, rating FROM Clothes ORDER BY rating LIMIT 1 )<br/>
                  OR<br/>
                  (SELECT Ad.elID FROM Ad, Electronics<br/>
                  WHERE Ad.elID IS NOT NULL AND Electronics.elID = Ad.elID IN (<br/>
                  SELECT elID, rating FROM Electronics ORDE RBY rating LIMIT 1 )<br/>
                  OR<br/>
                  (SELECT Ad.mID FROM Ad, MusicalInstruments<br/>
                  WHERE Ad.mID IS NOT NULL AND MusicalInstruments.mID = Ad.mID IN (<br/>
                  SELECT mID, rating FROM MusicalInstruments ORDER BY rating LIMIT 1 ) )
              </code>
                  <?php
          /*        // Attempt select query execution
                  $sql = "SELECT User.* FROM User
                          WHERE uID IN ((
                          (SELECT Ad.bID FROM Ad, Books
                          WHERE Ad.bID IS NOT NULL AND Books.bID = Ad.bID IN (
                          SELECT bID, rating FROM Books ORDER BY rating LIMIT 1 ) OR
                          (SELECT Ad.cID FROM Ad, Clothes
                          WHERE Ad.cID IS NOT NULL AND Clothes.cID = Ad.cID IN (
                          SELECT cID, rating FROM Clothes ORDER BY rating LIMIT 1 )
                          OR
                          (SELECT Ad.elID FROM Ad, Electronics
                          WHERE Ad.elID IS NOT NULL AND Electronics.elID = Ad.elID IN (
                          SELECT elID, rating FROM Electronics ORDE RBY rating LIMIT 1 )
                          OR
                          (SELECT Ad.mID FROM Ad, MusicalInstruments
                          WHERE Ad.mID IS NOT NULL AND MusicalInstruments.mID = Ad.mID IN (
                          SELECT mID, rating FROM MusicalInstruments ORDER BY rating LIMIT 1 ) )";
